param(
    ...
    [string]$OutputPath = "",
    [string]$FinalOwnerHex = "",
    [switch]$RequireDualCandidates,
    [switch]$RequireAllMarkers,
    [switch]$RequireStableClassifier,
    [switch]$AcknowledgeReadOnlyStall
)

$ErrorActionPreference = "Stop"
$projectRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
$binary = Join-Path $projectRoot `
    "build\staging\a9tas_successor_classifier_v1"
$parser = Join-Path $projectRoot "tools\parse_successor_classifier_v1.py"
$package = "com.aligames.kuang.kybc.aligames"
$remoteBinary = "/data/local/tmp/a9tas_successor_classifier_v1"

if (-not $AcknowledgeReadOnlyStall) {
    throw "This all-thread four-slot HWBP capture can visibly stall the game. Re-run only after explicit permission with -AcknowledgeReadOnlyStall."
}
if ($DurationMs -lt 100 -or $DurationMs -gt 15000) {
    throw "DurationMs must be between 100 and 15000"
}
if ($FinalOwnerHex -ne "" -and $FinalOwnerHex -notmatch '^[0-9a-fA-F]+$') {
    throw "FinalOwnerHex must be an unprefixed hexadecimal address"
}
if (-not (Test-Path -LiteralPath $AdbPath)) { throw "ADB not found: $AdbPath" }
if (-not (Test-Path -LiteralPath $binary)) { throw "Classifier not built: $binary" }
...
if ($OutputPath -eq "") {
    ...
    $OutputPath = Join-Path $projectRoot `
        "evidence\a9tas_successor_classifier_v1_$stamp.bin"
}
...
$remoteTrace = "/data/local/tmp/a9tas_successor_classifier_v1_$gamePid.bin"

& $AdbPath -s $Device push $binary $remoteBinary | Out-Host
if ($LASTEXITCODE -ne 0) { throw "Classifier push failed" }
& $AdbPath -s $Device shell "su -c 'chmod 700 $remoteBinary'"
if ($LASTEXITCODE -ne 0) { throw "Classifier chmod failed" }

Write-Host "Arming STRICTLY READ-ONLY four-slot successor classifier for $DurationMs ms..."
Write-Host "No game-memory writes, guest calls, ESC, or other input will be sent."
$remoteArguments = "$gamePid $baseHex $DurationMs $remoteTrace"
if ($FinalOwnerHex -ne "") { $remoteArguments += " $FinalOwnerHex" }
& $AdbPath -s $Device shell `
    "su -c '$remoteBinary $remoteArguments'" | Out-Host
if ($LASTEXITCODE -ne 0) { throw "Classifier execution failed" }
...
if ($tracerPid -ne "TracerPid:`t0" -and $tracerPid -ne "TracerPid: 0") {
    throw "Classifier did not detach cleanly: '$tracerPid'"
}

Write-Host "Trace: $OutputPath"
Write-Host "Maps: $mapsPath"
$parserArguments = @($parser, $OutputPath, "--require-window")
if ($RequireDualCandidates) { $parserArguments += "--require-dual-candidates" }
if ($RequireAllMarkers) { $parserArguments += "--require-all-markers" }
if ($RequireStableClassifier) {
    $parserArguments += "--require-stable-classifier"
}
python @parserArguments
if ($LASTEXITCODE -ne 0) {
    throw "Successor classifier assessment failed (exit=$LASTEXITCODE)"
}
