...
$source = Join-Path $root "src\hwbp_lifecycle_final_writer_replay_v1.cpp"
$policy = Join-Path $root "tools\test_lifecycle_final_writer_policy_v1.py"
$payload = Join-Path $root "build\final-writer-replay-v1\liba9tas_final_writer_replay_v1_build_only.so"
$outDir = Join-Path $root "build\lifecycle-final-writer-v1"
$object = Join-Path $outDir "lifecycle_final_writer_v1_review_only.o"
$candidate = Join-Path $outDir "a9tas_lifecycle_final_writer_v1_review_only"
$disassembly = Join-Path $outDir "lifecycle_final_writer_v1_review_only.disasm.txt"
...
foreach ($path in @($source, $policy, $payload, $compiler, $readelf, $objdump)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Missing lifecycle final-writer build input: $path"
    }
}
...
& $compiler $source "-O2" "-std=c++20" "-fno-exceptions" "-fno-rtti" `
    ...
    "-c" "-o" $object
if ($LASTEXITCODE -ne 0) { throw "Lifecycle final-writer review build failed" }

& $compiler $object "-static-libstdc++" "-Wl,--no-undefined" "-o" $candidate
if ($LASTEXITCODE -ne 0) { throw "Lifecycle final-writer retained link failed" }
$header = (& $readelf -h $candidate) -join "`n"
if ($header -notmatch "Machine:\s+Advanced Micro Devices X86-64" -or
    $header -notmatch "Type:\s+DYN") {
    throw "Lifecycle final-writer retained ELF mismatch"
}
& $objdump "-d" "--demangle" $object | Set-Content -LiteralPath $disassembly
if ($LASTEXITCODE -ne 0) { throw "Lifecycle final-writer disassembly failed" }
python -B $policy $object
if ($LASTEXITCODE -ne 0) { throw "Lifecycle final-writer policy failed" }
...
Write-Output "LIFECYCLE_FINAL_WRITER_BUILD passed=1 review_object=unlinked retained_candidate=review_only deployed=0 device_access=0"
