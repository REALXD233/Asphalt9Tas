...
$source = Join-Path $root "src\practice_mode_readonly_probe_v1.cpp"
$outDir = Join-Path $root "build\practice-mode-readonly-probe-v1"
...
foreach ($path in @($source, $readelf, $alignmentPolicy)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Missing practice probe build input: $path"
    }
}
. $alignmentPolicy
...
foreach ($target in $targets) {
    ...
    $name = "a9tas_practice_mode_readonly_probe_$($target.Machine)_v1"
    $output = Join-Path $outDir $name
    $pageFlags = @()
    if ($target.Machine -eq "arm64") {
        $pageFlags = @("-Wl,-z,max-page-size=16384", "-Wl,-z,common-page-size=16384")
    }
    & $compiler $source "-O2" "-std=c++20" "-fPIE" "-pie" `
        "-fno-exceptions" "-fno-rtti" "-static-libstdc++" `
        "-Wall" "-Wextra" "-Werror" "-Wl,--build-id=sha1" @pageFlags "-o" $output
    if ($LASTEXITCODE -ne 0) {
        throw "Practice probe build failed: $($target.Machine)"
    }
    $header = (& $readelf "-h" $output) -join "`n"
    if ($header -notmatch "Type:\s+DYN" -or
        $header -notmatch "Machine:\s+$([regex]::Escape($target.Elf))") {
        throw "Practice probe ABI mismatch: $($target.Machine)"
    }
    if ($target.Machine -eq "arm64") {
        Assert-A9TasElfLoadAlignment -ReadElf $readelf -ElfPath $output `
            -ExpectedAlignment 0x4000 -Label 'practice-mode probe arm64'
    }
    ...
    Write-Host "PRACTICE_PROBE machine=$($target.Machine) sha256=$sha"
}
