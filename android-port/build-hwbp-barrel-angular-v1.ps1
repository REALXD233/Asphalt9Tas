...
$source = Join-Path $projectRoot "src\hwbp_barrel_angular_observer_v1.cpp"
$outputDirectory = Join-Path $projectRoot "build\staging"
$output = Join-Path $outputDirectory "a9tas_hwbp_barrel_angular_observer_v1"
...
if ($LASTEXITCODE -ne 0) {
    throw "barrel-angular observer build failed (exit=$LASTEXITCODE)"
}
