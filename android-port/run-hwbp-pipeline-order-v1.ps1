...
$binary = Join-Path $projectRoot `
    "build\staging\a9tas_hwbp_pipeline_order_observer_v1"
$parser = Join-Path $projectRoot "tools\parse_hwbp_pipeline_order_v1.py"
$package = "com.aligames.kuang.kybc.aligames"
$remoteBinary = "/data/local/tmp/a9tas_hwbp_pipeline_order_observer_v1"

if (-not $AcknowledgeCaptureStall) {
    throw "This all-thread HWBP capture will visibly stall the game. Re-run only after explicit user permission with -AcknowledgeCaptureStall."
}
...
if ($OutputPath -eq "") {
    ...
    $OutputPath = Join-Path $projectRoot `
        "evidence\a9tas_hwbp_pipeline_order_v1_$stamp.bin"
}
...
$remoteTrace = "/data/local/tmp/a9tas_hwbp_pipeline_order_v1_$gamePid.bin"
...
Write-Host "Arming host-only four-address pipeline capture for $DurationMs ms..."
& $AdbPath -s $Device shell `
    "su -c '$remoteBinary $gamePid $baseHex $DurationMs $remoteTrace $PhysicsContextHex'" `
    | Out-Host
...
python $parser $OutputPath --minimum-cycles 20 --require-supported
if ($LASTEXITCODE -ne 0) {
    throw "Pipeline-order semantic assessment failed (exit=$LASTEXITCODE)"
}
