...
$source = Join-Path $projectRoot "src\hwbp_conditional_executor_v1.cpp"
$outputDirectory = Join-Path $projectRoot "build\staging"
$output = Join-Path $outputDirectory "a9tas_hwbp_conditional_executor_v1"
...
if ($LASTEXITCODE -ne 0) {
    throw "conditional executor build failed (exit=$LASTEXITCODE)"
}
