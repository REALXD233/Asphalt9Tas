...
void DumpDispatchList(std::uintptr_t guest_base, const char* name,
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        __android_log_print(ANDROID_LOG_INFO, kTag,
                            "    %s sub[%zu] elem=0x%llx vtable=0x%llx(off 0x%llx) "
                            "cb=0x%llx(off 0x%llx)",
                            name, i, static_cast<unsigned long long>(raw),
                            static_cast<unsigned long long>(vtable),
                            static_cast<unsigned long long>(vtable - guest_base),
