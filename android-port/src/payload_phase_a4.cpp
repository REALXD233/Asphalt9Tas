...
void LogPhysicsStructure(std::uintptr_t guest_base) {
    ...
    log_ptr("dispatch[+0x180]+0", ctx + 0x180);
    log_ptr("dispatch[+0x180]+8", ctx + 0x188);
    log_ptr("dispatch+0x28", ctx + 0x180 + 0x28);
    log_ptr("dispatch+0x30", ctx + 0x180 + 0x30);
    // walk the subscriber list: begin=[+0x180], end=[+0x188], 16-byte elems
    std::uintptr_t begin = 0, end = 0;
    std::memcpy(&begin, reinterpret_cast<const void*>(ctx + 0x180), sizeof(begin));
    std::memcpy(&end, reinterpret_cast<const void*>(ctx + 0x188), sizeof(end));
    if (begin == 0 || end <= begin || (end - begin) % 16 != 0 ||
        (end - begin) / 16 > 64) {
        __android_log_print(ANDROID_LOG_WARN, kTag,
                            "  subscriber list range odd begin=%p end=%p",
                            reinterpret_cast<void*>(begin), reinterpret_cast<void*>(end));
        return;
    }
    const size_t count = (end - begin) / 16;
    __android_log_print(ANDROID_LOG_INFO, kTag, "  subscribers count=%zu", count);
    for (size_t i = 0; i < count; ++i) {
        // element at begin+8+i*16 (loop starts at x22=8, steps 0x10)
        std::uintptr_t raw = 0;
        std::memcpy(&raw, reinterpret_cast<const void*>(begin + 8 + i * 16),
                    sizeof(raw));
        __android_log_print(ANDROID_LOG_INFO, kTag,
                            "    sub[%zu] raw=0x%llx (off 0x%llx)",
                            i, static_cast<unsigned long long>(raw),
                            static_cast<unsigned long long>(raw - guest_base));
    }
}
