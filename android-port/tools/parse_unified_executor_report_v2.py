#!/usr/bin/env python3
"""Strict offline verifier for successful A9UER2 unified-executor reports."""
...
GATE2_COMPLETE = 1 << 5
COMMITTED = 1 << 6
PREFIX_CERTIFIED = 1 << 7
SUPPORTED_FRAME_FLAGS = 0xFF
CORRECTION_MASK = CORRECTION_EQUAL | CORRECTION_CORRECTED | CORRECTION_SKIPPED
REQUIRED_FRAME_FLAGS = AUDIT_EXACT | GATE2_COMPLETE | COMMITTED | PREFIX_CERTIFIED
...
_FRAME = struct.Struct("<QQiIqq6Q2QH6s64s12s804s804s")
assert _HEADER.size == HEADER_SIZE
assert _FRAME.size == FRAME_SIZE


@dataclass(frozen=True)
class UnifiedReportSummaryV2:
    frames: int
    first_tick: int
    last_tick: int
...
def decode_report(blob: bytes) -> UnifiedReportSummaryV2:
    if len(blob) < HEADER_SIZE:
        raise ValueError("report is shorter than A9UER2 header")
    ...
    if magic != MAGIC or version != VERSION:
        raise ValueError("unsupported A9UER2 magic/version")
    if header_size != HEADER_SIZE or frame_size != FRAME_SIZE:
        raise ValueError("A9UER2 ABI size mismatch")
    if flags != REQUIRED_HEADER_FLAGS or reserved0 != 0:
        raise ValueError("A9UER2 flags/reserved mismatch")
    if build_id != BUILD_ID:
        raise ValueError("A9UER2 Build ID mismatch")
    if recording_frames < 1 or processed_frames != recording_frames:
        raise ValueError("A9UER2 incomplete frame count")
    expected_size = HEADER_SIZE + recording_frames * FRAME_SIZE
    if len(blob) != expected_size:
        raise ValueError(f"A9UER2 length must be exactly {expected_size} bytes")
    addresses = (
        pid,
        library_base,
        main_object,
    ...
    if any(value == 0 for value in addresses):
        raise ValueError("A9UER2 contains a zero identity/address")
    if transform_address != native_body + TRANSFORM_OFFSET or linear_address != native_body + LINEAR_OFFSET:
        raise ValueError("A9UER2 native payload address mismatch")
    if any((read_errors, ptrace_errors, semantic_errors, write_failures, rollback_attempts, rollback_failures)):
        raise ValueError("A9UER2 success report contains an error/rollback")
    if equal_frames + corrected_frames + skipped_frames != recording_frames:
        raise ValueError("A9UER2 correction counters do not cover all frames")
    if write_attempts != corrected_frames:
        raise ValueError("A9UER2 write-attempt count mismatch")
    if delta_writes != recording_frames:
        raise ValueError("A9UER2 fixed-delta write count mismatch")
    if initial_threads < 1 or final_threads != initial_threads + thread_additions:
        raise ValueError("A9UER2 thread detach accounting mismatch")
    ...
    for index in range(recording_frames):
        ...
        if not (previous_commit < events[0] < events[1] < events[2] < events[3] < events[4] < events[5] <= event_count):
            raise ValueError(f"frame {index}: certified phase event order mismatch")
        if completion_before == completion_after:
            raise ValueError(f"frame {index}: completion token did not change before C9C")
        if callback_flags_at_c9c & 0xFF != 1:
    ...
    if (counted_equal, counted_corrected, counted_skipped) != (equal_frames, corrected_frames, skipped_frames):
        raise ValueError("A9UER2 header/frame correction count mismatch")
    if control_writes != counted_steering * 2:
        raise ValueError("A9UER2 steering write count mismatch")
    assert fixed_interval is not None and previous_tick is not None
    return UnifiedReportSummaryV2(
        recording_frames,
...
def main() -> int:
    ...
    try:
        ...
        print(f"a9uer2_error={error}")
        return 1
    print(
        f"a9uer2_supported=1 frames={summary.frames} "
