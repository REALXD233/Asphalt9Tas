...
$source = Join-Path $projectRoot "src\hwbp_pipeline_order_observer_v1.cpp"
$outputDirectory = Join-Path $projectRoot "build\staging"
$output = Join-Path $outputDirectory "a9tas_hwbp_pipeline_order_observer_v1"
...
if ($LASTEXITCODE -ne 0) {
    throw "pipeline-order observer build failed (exit=$LASTEXITCODE)"
}
