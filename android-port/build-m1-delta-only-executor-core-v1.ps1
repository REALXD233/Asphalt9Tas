...
$header = Join-Path $root "src\m1_delta_only_executor_core_v1.h"
$selftest = Join-Path $root "src\m1_delta_only_executor_core_selftest_v1.cpp"
$outDir = Join-Path $root "build\m1-delta-only-executor-core-v1"
$armObject = Join-Path $outDir "m1_delta_only_executor_core_arm64_v1.o"
$hostObject = Join-Path $outDir "m1_delta_only_executor_core_x86_64_v1.o"

foreach ($path in @($armCompiler, $hostCompiler, $header, $selftest)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Missing M1 delta-only core input: $path"
    }
}
...
foreach ($build in @(
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "M1 delta-only core build failed: $($build[1])"
    }
}
...
Write-Output "M1_DELTA_ONLY_EXECUTOR_CORE_BUILD passed=1 arm64=1 x86_64=1 owns_fixed_delta_only=1 control_writes=0 action_writes=0 physics_writes=0 device_access=0 deployed=0"
