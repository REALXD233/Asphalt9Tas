...
$outputDir = Join-Path $projectRoot "build\producer-thread-nativebridge-probe-v1"
...
$payload = Join-Path $outputDir "liba9tas_producer_thread_probe_v1.so"
...
if ($LASTEXITCODE -ne 0) { throw "producer-thread ARM64 probe build failed" }

$bootstrap = Join-Path $outputDir "liba9tas_bootstrap_producer_thread_probe_v1.so"
& $x64Compiler "-shared" "-fPIC" "-O2" "-std=c++20" "-static-libstdc++" `
    "-Wall" "-Wextra" "-Werror" "-Wl,--build-id=sha1" `
    (Join-Path $sourceDir "bootstrap_producer_thread_probe_v1_build.cpp") `
    "-llog" "-ldl" "-o" $bootstrap
if ($LASTEXITCODE -ne 0) { throw "producer-thread x86_64 bootstrap build failed" }

$controller = Join-Path $outputDir "a9tas_producer_thread_nativebridge_probe_controller_v1"
& $x64Compiler "-O2" "-std=c++20" "-static-libstdc++" `
    "-Wall" "-Wextra" "-Werror" "-Wno-unused-function" `
    (Join-Path $sourceDir "producer_thread_nativebridge_probe_controller_v1.cpp") `
    "-ldl" "-o" $controller
if ($LASTEXITCODE -ne 0) { throw "producer-thread controller build failed" }
...
if ($controllerHeader -notmatch "Advanced Micro Devices X86-64") {
    throw "controller is not x86-64"
}
...
if ($payloadSymbols -notmatch "a9tas_same_thread_probe_v1") {
    throw "ARM64 gettid export missing"
}
...
if ($bootstrapSymbols -notmatch "a9tas_bootstrap_same_thread_probe_status") {
    throw "bootstrap status getter missing"
}

python (Join-Path $projectRoot "tools\test_producer_thread_nativebridge_probe_policy_v1.py")
if ($LASTEXITCODE -ne 0) { throw "producer-thread policy tests failed" }

function Get-Hash256([string]$Path) {
    return (Get-FileHash -LiteralPath $Path -Algorithm SHA256).Hash.ToLowerInvariant()
}

Write-Output "PT_NB0_BUILD_ONLY passed=1 deployed=0 selftest=compiled_not_run guest_calls=0 game_calls=0 gameplay_writes=0"
