...
$source = Join-Path $projectRoot "src\barrel_rbx_candidate_sampler_v1.cpp"
$outputDirectory = Join-Path $projectRoot "build\staging"
$output = Join-Path $outputDirectory "a9tas_barrel_rbx_candidate_sampler_v1"
...
if ($LASTEXITCODE -ne 0) {
    throw "barrel RBX candidate sampler build failed (exit=$LASTEXITCODE)"
}
