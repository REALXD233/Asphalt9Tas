...
$source = Join-Path $projectRoot "src\payload_source_proxy_replay_v1.cpp"
...
$output = Join-Path $outputDir "liba9tas_payload_source_proxy_replay_v1.so"
...
& $compiler "-shared" "-fPIC" "-O2" "-std=c++20" "-static-libstdc++" `
    ...
    "-Wl,--gc-sections" $source "-llog" "-ldl" "-o" $output
if ($LASTEXITCODE -ne 0) { throw "source-proxy v1 payload build failed" }
