...
$source = Join-Path $projectRoot "src\unified_tick_executor_core_v1.cpp"
$outputDirectory = Join-Path $projectRoot "build\staging"
$output = Join-Path $outputDirectory "a9tas_unified_tick_executor_core_v1"
...
if ($LASTEXITCODE -ne 0) {
    throw "unified tick executor core build failed (exit=$LASTEXITCODE)"
}
