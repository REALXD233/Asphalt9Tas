# Guarded A9PHEX1 gate: register the natural-action callback, prove the complete
# runtime graph read-only, then replay exactly five phase-paced frames.
...
param(
    ...
    [switch]$AcknowledgeFiveFramePhasePacedWrites,
...
$buildScripts = @(
    ...
    (Join-Path $root "build-controller-shadow-phase-paced-executor-v1.ps1"),
...
$executor = Join-Path $root "build\controller-shadow-phase-paced-executor-v1\a9tas_controller_shadow_phase_paced_executor_v1_reviewed"
...
$phaseParser = Join-Path $root "tools\parse_phase_paced_executor_report_v1.py"
$actionValidator = Join-Path $root "tools\validate_natural_action_external_replay_report_v1.py"
$policy = Join-Path $root "tools\test_run_controller_shadow_phase_paced_gate_policy_v1.py"
$candidateVerifier = Join-Path $root "tools\verify_phase_paced_five_frame_candidate_v1.py"
$receipt = Join-Path $root "build\controller-shadow-phase-paced-gate-v1\prepared-process-v1.json"
...
$pins = @{
    ...
    $executor = "c60df0f52dc49abb1e2d413518ad5ddd4c6431c156434be121047a8e12ce373e"
    ...
    $phaseParser = "78c0ec1f2a88c4d551f0a6e9e49d2aea21cec8c6d036407fe3ba0a933e306e17"
...
$remoteExecutor = "/data/local/tmp/a9tas_controller_shadow_phase_paced_executor_v1"
...
$executorAck = "I_ACCEPT_PHASE_PACED_EXECUTOR_REVIEW_V1"
...
function Invoke-RemoteReadOnlyWithSnapshotRetry([string]$Command, [string]$Label) {
    for ($attempt = 1; $attempt -le 3; $attempt++) {
        $result = & $AdbPath -s $Device shell "su -c '$Command'" 2>&1
        $text = (($result | Out-String).Trim())
        if ($LASTEXITCODE -eq 0) { return $text }
        $transientSnapshotRace =
            $text -match 'stage=gameplay_input_controller detail=result_-4 '
        if (-not $transientSnapshotRace -or $attempt -eq 3) {
            throw "$Label failed attempt=${attempt}: $text"
        }
        Start-Sleep -Milliseconds 50
    }
    throw "$Label exhausted read-only snapshot retries"
}
...
foreach ($path in @($buildScripts + @($policy, $candidateVerifier))) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Missing M1 build/policy input: $path"
    }
}
python -B $policy $MyInvocation.MyCommand.Path
if ($LASTEXITCODE -ne 0) { throw "A9PHEX1 runner policy failed" }
if ($Mode -eq 'OfflineValidate') {
    foreach ($script in $buildScripts) {
        & $script | Out-Host
        if ($LASTEXITCODE -ne 0) { throw "A9PHEX1 offline build failed: $script" }
    }
    Assert-LocalPins
    python -B $candidateVerifier
    if ($LASTEXITCODE -ne 0) { throw "A9PHEX1 candidate manifest failed" }
    Write-Output "PHASE_PACED_GATE_OFFLINE passed=1 frames=5 boundaries=4 lifecycle_gate=2_to_3 frozen_ready_gate=1 fixed_delta_writes=5 controller_payload=1 final_writer=1 natural_action=1 deployed=0 device_access=0"
    return
}

# The first live A9PHEX1 gate proved that the uniquely resolved controller
# object did not enter its UpdatePerTick wrapper before the active C98 phase.
# Do not permit another device attempt by moving receipt checks or guessing a
# later boundary. Preserve the offline artifacts and failed report only.
throw "A9PHEX1 live modes are retired after the five-frame receipt-order failure"
...
if ($Mode -eq 'ReadOnlyBasePreflight') {
    ...
    foreach ($gate in @(
        ...
        @($AcknowledgeFiveFramePhasePacedWrites, 'five phase-paced fixed-delta frames plus payload-owned control, action, and conditional writer work'),
...
$remotePhaseReport = "/data/local/tmp/a9tas_phase_${gamePid}_$stamp.a9phex1"
$remoteActionReport = "/data/local/tmp/a9tas_phase_${gamePid}_$stamp.a9nar6"
$nalReady = "/data/local/tmp/a9tas_phase_nal_ready_${gamePid}_$stamp"
$nalArm = "/data/local/tmp/a9tas_phase_nal_arm_${gamePid}_$stamp"
$externalReady = "/data/local/tmp/a9tas_phase_external_${gamePid}_$stamp"
$phaseReady = "/data/local/tmp/a9tas_phase_ready_${gamePid}_$stamp"
$escDispatched = "/data/local/tmp/a9tas_phase_esc_dispatched_${gamePid}_$stamp"
$localPhaseReport = Join-Path $OutputDirectory "a9tas_phase_gate_5f_$stamp.a9phex1"
$localActionReport = Join-Path $OutputDirectory "a9tas_phase_gate_5f_$stamp.a9nar6"
...
if ($Mode -eq 'ReadOnlyBasePreflight') {
    try {
        ...
        $basePreflightText = Invoke-RemoteReadOnlyWithSnapshotRetry "$remotePreflight $gamePid $base $startTicks $remoteRecording $remoteTarget $basePreflightAck" 'M1 base read-only preflight'
...
try {
    ...
    $executorCommand = "$remoteExecutor $gamePid $base $startTicks $TimeoutMs $remoteRecording $remoteTarget $remotePhaseReport $phaseReady $executorAck"
    $executorHandle = Start-Remote $executorCommand
    if (-not (Wait-RemoteFile $phaseReady 15000)) { throw "A9PHEX1 executor did not reach frozen READY" }
    $phaseReadyText = Invoke-AdbText @('-s', $Device, 'shell', "su -c 'cat $phaseReady'")
    if ($phaseReadyText -notmatch '^PHASE_PACED_READY_ARMED ' -or
        $phaseReadyText -notmatch 'state=2 ' -or
        $phaseReadyText -notmatch 'delta=0x[0-9a-f]+ c98=0x[0-9a-f]+ c9c=0x[0-9a-f]+ world=0x[0-9a-f]+ controller=0x[0-9a-f]+ ' -or
        $phaseReadyText -notmatch 'frames=5 writer_installed=1 controller_installed=1 ' -or
        $phaseReadyText -notmatch 'all_target_threads_frozen=1 delta_writes=0 host_resume_gate=marker_removal controller_pid=(\d+)$') {
        throw "A9PHEX1 frozen READY proof mismatch: $phaseReadyText"
    }
    $controllerPid = [int]$matches[1]
    if ((Get-TracerPid $gamePid) -ne $controllerPid) { throw "A9PHEX1 tracer identity mismatch" }

    $escHandle = Start-Remote "input keyevent 111 & echo PHASE_ESC_DISPATCHED $gamePid $startTicks > $escDispatched; wait"
    ...
    if ($escProof -ne "PHASE_ESC_DISPATCHED $gamePid $startTicks") { throw "Final resume ESC dispatch proof mismatch" }
    Invoke-AdbChecked @('-s', $Device, 'shell', "su -c 'rm -f $phaseReady'") 'release A9PHEX1 frozen gate' | Out-Null
    ...
    $phaseReportPulled = $false
    try {
        Invoke-AdbChecked @('-s', $Device, 'pull', $remotePhaseReport, $localPhaseReport) 'pull A9PHEX1 report' | Out-Null
        $phaseReportPulled = $true
    } catch {
        Write-Warning "A9PHEX1 report could not be preserved: $($_.Exception.Message)"
    }
    if ($executorResult.ExitCode -ne 0 -or
        $executorResult.Stdout -notmatch 'PHASE_PACED_EXECUTOR_DONE success=1 frames=5 ' -or
        $executorResult.Stdout -notmatch 'delta_writes=5 ') {
        $suffix = if ($phaseReportPulled) { " local_report=$localPhaseReport" } else { " local_report=unavailable" }
        throw "A9PHEX1 executor failed closed.$suffix"
    }
    if ((Get-TracerPid $gamePid) -ne 0) { throw "A9PHEX1 executor did not detach" }
    ...
    if (-not $phaseReportPulled) {
        Invoke-AdbChecked @('-s', $Device, 'pull', $remotePhaseReport, $localPhaseReport) 'pull A9PHEX1 report' | Out-Null
    }
    Invoke-AdbChecked @('-s', $Device, 'pull', $remoteActionReport, $localActionReport) 'pull natural report' | Out-Null
    python -B $phaseParser $localPhaseReport $recording $controllerPayload --frames 5
    if ($LASTEXITCODE -ne 0) { throw "A9PHEX1 report validation failed" }
    python -B $actionValidator $localActionReport $recording
    if ($LASTEXITCODE -ne 0) { throw "Natural action report validation failed" }
    $success = $true
    Write-Output "PHASE_PACED_GATE_PASSED pid=$gamePid start_ticks=$startTicks frames=5 delta_writes=5 boundaries=delta+c98+c9c+world lifecycle=2_to_3 receipts=controller+writer+action cleanup=controller+writer+natural+detach"
    Write-Output "phase_report=$localPhaseReport"
    Write-Output "natural_report=$localActionReport"
}
finally {
    foreach ($path in @($nalReady, $nalArm, $externalReady, $phaseReady,
                         $escDispatched,
    ...
    if (-not $success) {
        foreach ($entry in @(
            @($executorHandle, 'A9PHEX1 executor', $executorResultConsumed),
        ...
        try {
            if ((Invoke-AdbText @('-s', $Device, 'shell', "su -c 'test -f $remotePhaseReport; echo `$?'")) -eq '0') {
                Invoke-AdbChecked @('-s', $Device, 'pull', $remotePhaseReport, $localPhaseReport) 'preserve failed A9PHEX1 executor report' | Out-Null
                Write-Warning "Preserved failed report: $localPhaseReport"
            }
        } catch {}
        try {
    ...
    foreach ($path in @($remotePhaseReport, $remoteActionReport)) {
