...
$source = Join-Path $projectRoot "src\hwbp_worker_boundary_observer_v1.cpp"
$outputDirectory = Join-Path $projectRoot "build\staging"
$output = Join-Path $outputDirectory "a9tas_hwbp_worker_boundary_observer_v1"
...
if ($LASTEXITCODE -ne 0) {
    throw "worker-boundary observer build failed (exit=$LASTEXITCODE)"
}
