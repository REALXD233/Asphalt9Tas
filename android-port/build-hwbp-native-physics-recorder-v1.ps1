...
$source = Join-Path $projectRoot "src\hwbp_native_physics_recorder_v1.cpp"
$outputDirectory = Join-Path $projectRoot "build\staging"
$output = Join-Path $outputDirectory "a9tas_hwbp_native_physics_recorder_v1"
...
if ($LASTEXITCODE -ne 0) {
    throw "native-physics recorder build failed (exit=$LASTEXITCODE)"
}
