# GUARDED ONE-FRAME STEERING-ONLY LIVE RUNNER. This file is not authorization.
...
$inputVerifier = Join-Path $projectRoot `
    "tools\verify_unified_steering_gate_v1.py"
$resultVerifier = Join-Path $projectRoot `
    "tools\verify_gate7_steering_result_v1.py"
$expectedBinaryHash = `
...
if ($inputExitCode -ne 0) {
    throw "Gate 7 steering A9UTK1/manifest validation failed"
}
$inputText = $inputAssessment -join [Environment]::NewLine
$hashMatch = [regex]::Match($inputText, 'sha256=([0-9a-f]{64})')
if (-not $hashMatch.Success) {
    throw "Could not recover validated Gate 7 input SHA256"
}
$actualRecordingHash = Get-LocalSha256 $RecordingPath
if ($actualRecordingHash -ne $hashMatch.Groups[1].Value) {
    throw "Gate 7 input hash changed after validation"
}

if ($OfflineValidateOnly) {
    Write-Host "GATE7_STEERING_OFFLINE_VALIDATION_OK"
    Write-Host "frames=1 skip_flags=0xfe steering_writes=2 final_writes=0 fixed_delta_writes=1"
    Write-Host "recording_sha256=$actualRecordingHash"
    Write-Host "executor_sha256=$actualBinaryHash"
    return
}

if (-not $Gate6Validated) {
    throw "Gate 6 A9UER4 live evidence must be acknowledged with -Gate6Validated."
}
...
if (-not $AcknowledgeTwoSteeringWrites) {
    throw "This gate writes steering at C98 and C9C. Pass -AcknowledgeTwoSteeringWrites."
}
if (-not $AcknowledgeSingleFrameNoTrajectoryClaim) {
    throw "This is one steering integration frame, not trajectory proof. Pass -AcknowledgeSingleFrameNoTrajectoryClaim."
}
...
if ($OutputPath -eq "") {
    $OutputPath = Join-Path $projectRoot `
        "evidence\a9tas_gate7_steering_report_$stamp.a9uer5"
}
...
New-Item -ItemType Directory -Path (Split-Path -Parent $OutputPath) `
    -Force | Out-Null
$remoteRecording = "/data/local/tmp/a9tas_gate7_steering_${gamePid}_$stamp.a9utk1"
$remoteReport = "/data/local/tmp/a9tas_gate7_steering_${gamePid}_$stamp.a9uer5"
...
function Assert-StableGameAndDetach {
    ...
    if ($LASTEXITCODE -ne 0 -or $pidAfter -ne $pidText) {
        throw "Game process exited or changed PID during Gate 7"
    }
    $tracerPid = (& $AdbPath -s $Device shell `
        "cat /proc/$gamePid/status | grep '^TracerPid:'").Trim()
    if ($tracerPid -ne "TracerPid:`t0" -and $tracerPid -ne "TracerPid: 0") {
        throw "Gate 7 left a tracer attached: '$tracerPid'"
    }
}
...
Write-Host "GATE7_STEERING_ARMING frames=1 pid=$gamePid steering_writes=2"
...
if ($executionCode -ne 0) {
    throw "Gate 7 executor failed closed (exit=$executionCode, TracerPid=0)"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Gate 7 steering acceptance failed"
}

Write-Host "GATE7_UNIFIED_STEERING_PASSED"
