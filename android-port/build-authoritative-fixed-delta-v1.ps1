...
$neutralSource = Join-Path $root "src\hwbp_authoritative_neutral_observer_v1.cpp"
...
$policy = Join-Path $root "tools\test_authoritative_fixed_delta_cpp_policy_v1.py"
$outDir = Join-Path $root "build\authoritative-fixed-delta-v1"
New-Item -ItemType Directory -Path $outDir -Force | Out-Null

foreach ($binding in @(
    @($neutralSource, "3ad4aac949fa9fdc74ec517e7fb5bdd3f843d5a0d460e6a9a5fcf0fa9242058d", "frozen neutral LIVE PASS source"),
...
$candidate = Join-Path $outDir "a9tas_hwbp_authoritative_fixed_delta_v1"
$reviewObject = Join-Path $outDir "hwbp_authoritative_fixed_delta_v1.o"
$disassembly = Join-Path $outDir "hwbp_authoritative_fixed_delta_v1.disasm.txt"
...
if ($LASTEXITCODE -ne 0) { throw "authoritative fixed-delta build failed" }

& $compiler $source "-O2" "-std=c++20" "-fno-exceptions" "-fno-rtti" `
    "-Wall" "-Wextra" "-Werror" "-c" "-o" $reviewObject
if ($LASTEXITCODE -ne 0) { throw "authoritative fixed-delta object build failed" }

& $objdump "-d" "--demangle" $reviewObject | Set-Content -LiteralPath $disassembly
if ($LASTEXITCODE -ne 0) { throw "authoritative fixed-delta disassembly failed" }

$python = Get-Command python.exe -ErrorAction Stop
& $python.Source $policy $candidate $reviewObject $readelf $objdump
if ($LASTEXITCODE -ne 0) { throw "authoritative fixed-delta policy failed" }
...
Write-Output "AUTHORITATIVE_FIXED_DELTA_BUILD passed=1 runtime_candidate=1 deployed=0 device_access=0 delta_callsite=1 all_other_capabilities=skipped"
