...
$output = Join-Path $outputDirectory "a9tas_hwbp_synchronized_tick_recorder_v1"
...
# The source intentionally includes the proven executor translation unit so
# both binaries use one phase machine. Main-only executor helpers are therefore
# unused in this recorder translation unit.
$arguments = @(
...
if ($LASTEXITCODE -ne 0) {
    throw "synchronized HWBP tick recorder build failed (exit=$LASTEXITCODE)"
}
