...
$header = Join-Path $root "src\camera_shape_state_v1.h"
$selftest = Join-Path $root "src\camera_shape_state_selftest_v1.cpp"
$source = Join-Path $root "src\camera_shape_state_sampler_v1.cpp"
$policy = Join-Path $root "tools\test_camera_shape_state_policy_v1.py"
$outDir = Join-Path $root "build\camera-shape-state-v1"
...
$armObject = Join-Path $outDir "camera_shape_state_v1_arm64_review_only.o"
$x86Object = Join-Path $outDir "camera_shape_state_v1_x86_64_review_only.o"
$output = Join-Path $outDir "a9tas_camera_shape_state_sampler_v1_review_only"

foreach ($path in @($header, $selftest, $source, $policy, $arm, $x86)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Missing camera shape-state input: $path"
    }
}
...
foreach ($pair in @(@($arm, $armObject), @($x86, $x86Object))) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Camera shape-state selftest build failed" }
}
...
if ($LASTEXITCODE -ne 0) { throw "Camera shape-state sampler build failed" }

python -B $policy $output
if ($LASTEXITCODE -ne 0) { throw "Camera shape-state policy failed" }

Write-Output "CAMERA_SHAPE_STATE_BUILD passed=1 read_only=1 arm64=1 x86_64=1 device_access=0 deployed=0"
