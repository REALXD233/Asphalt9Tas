...
$outDir = Join-Path $root "build\camera-tool-v1"
...
$source = Join-Path $root "src\payload_camera_tool_v1.cpp"
$protocol = Join-Path $root "src\camera_tool_protocol_v1.h"
$policy = Join-Path $root "tools\test_camera_tool_payload_policy_v1.py"
$output = Join-Path $outDir "liba9tas_camera_tool_v1_build_only.so"
$disassembly = Join-Path $outDir "camera_tool_v1.disasm.txt"
foreach ($path in @($compiler,$readelf,$objdump,$source,$protocol,$policy)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Missing Camera Tool input: $path"
    }
}
...
if ($LASTEXITCODE -ne 0) { throw "Camera Tool payload build failed" }
& $objdump -d --demangle $output | Set-Content -LiteralPath $disassembly
if ($LASTEXITCODE -ne 0) { throw "Camera Tool disassembly failed" }
python -B $policy $output $readelf $objdump
if ($LASTEXITCODE -ne 0) { throw "Camera Tool policy failed" }
Write-Output "CAMERA_TOOL_BUILD passed=1 build_only=1 arm64=1 deployed=0 device_access=0"
