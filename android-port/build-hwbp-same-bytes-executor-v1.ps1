...
$source = Join-Path $projectRoot "src\hwbp_same_bytes_executor_v1.cpp"
$outputDirectory = Join-Path $projectRoot "build\staging"
$output = Join-Path $outputDirectory "a9tas_hwbp_same_bytes_executor_v1"
...
if ($LASTEXITCODE -ne 0) {
    throw "same-bytes executor build failed (exit=$LASTEXITCODE)"
}
