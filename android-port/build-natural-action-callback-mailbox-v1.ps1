...
$source = Join-Path $root "src\natural_action_callback_mailbox_selftest_v1.cpp"
$outDir = Join-Path $root "build\natural-action-callback-mailbox-v1"
...
$x64Out = Join-Path $outDir "a9tas_natural_action_callback_mailbox_selftest_v1"
$arm64Object = Join-Path $outDir "natural_action_callback_mailbox_selftest_v1.arm64.o"

& $x64 $source @common "-static" "-fPIE" "-o" $x64Out
if ($LASTEXITCODE -ne 0) { throw "x86_64 natural action mailbox build failed" }
& $arm64 $source @common "-c" "-o" $arm64Object
if ($LASTEXITCODE -ne 0) { throw "ARM64 natural action mailbox compile check failed" }
...
& $python.Source (Join-Path $root "tools\test_natural_action_callback_mailbox_v1.py")
if ($LASTEXITCODE -ne 0) { throw "Natural action mailbox model tests failed" }
...
Write-Output "NATURAL_ACTION_CALLBACK_MAILBOX_V1_BUILD passed=1 python_tests=5"
